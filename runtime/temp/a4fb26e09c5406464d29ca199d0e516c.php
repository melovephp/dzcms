<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:31:"./admintpl/admin_rule_list.html";i:1508376511;}*/ ?>
﻿<!DOCTYPE HTML>
<html>
<head>
<meta charset="utf-8">
		...
		<article class="cl pd-20">
			...
			<div class="text-c" style="height: 24px;">
				<form action="/admin-permission.html" method="post" style="float: right;">
					<input type="text" class="input-text" style="width:250px" placeholder="输入权限名称" name="content" value="<?php if(!(empty($val) || (($val instanceof \think\Collection || $val instanceof \think\Paginator ) && $val->isEmpty()))): ?><?php echo $val; endif; ?>">
			...
			<table class="table table-border table-bordered table-bg">
				...
				<?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): if( count($list)==0 ) : echo "" ;else: foreach($list as $key=>$list): ?>
					<tr class="text-c">
						...
						<td><?php if($list['level'] == 1): ?> 模块
							<?php elseif($list['level'] == 2): ?> 控制器
							<?php elseif($list['level'] == 3): ?> 方法
							<?php endif; ?>
						</td>
						<td class="td-status"><span class="label  radius <?php if($list['status']): ?> label-success<?php else: ?> label-default<?php endif; ?>"><?php echo !empty($list['status'])?"已启用":"已禁用"; ?></span></td>
						<td class="td-manage">
							<?php if($list['status']): ?>
							<a style="text-decoration:none" id="status" onClick="admin_permission__stop(this,<?php echo $list['id']; ?>)" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a>
							<?php else: ?>
							<a style="text-decoration:none" id="status" onClick="admin_permission__start(this,<?php echo $list['id']; ?>)" href="javascript:;" title="启用"><i class="Hui-iconfont">&#xe615;</i></a>
							<?php endif; ?>
							<a title="编辑" href="javascript:;" onclick="admin_permission_edit('权限编辑','admin-permission-edit?id=<?php echo $list['id']; ?>','800','500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
							<a title="删除" href="javascript:;" onclick="admin_permission_del(<?php echo $list['id']; ?>,'one')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
